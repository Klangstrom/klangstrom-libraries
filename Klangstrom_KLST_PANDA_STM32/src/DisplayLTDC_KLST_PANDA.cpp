...
inline volatile uint32_t LTDC_get_backbuffer_address(void) {
...
void display_line(const uint32_t x, const uint32_t y, const uint32_t length, const uint32_t color) {
    const uint32_t offset      = (x + y * KLST_DISPLAY_WIDTH) * BYTES_PER_PIXEL;
    auto           destination = reinterpret_cast<uint32_t*>(LTDC_get_backbuffer_address() + offset);
